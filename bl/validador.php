...
$pasta_recibos = 'recibos_temp';

if (!is_dir($pasta_recibos)) { mkdir($pasta_recibos, 0777, true); }

$arquivos = glob("$pasta_recibos/*.json");
foreach ($arquivos as $arquivo) {
    if (is_file($arquivo)) {
        if (time() - filemtime($arquivo) > 1800) { unlink($arquivo); }
    }
}
...
if (isset($_GET['ver'])) {
    ...
    $caminho_arquivo = "$pasta_recibos/$id_recibo.json";

    if (file_exists($caminho_arquivo)) {
        $conteudo = json_decode(file_get_contents($caminho_arquivo), true);
        if ((time() - $conteudo['timestamp']) > 1200) {
        ...
        $mensagem_erro = "❌ Relatório não encontrado.";
    }
}
?>
...
<head>
    ...
    <title><?php echo $modo_visualizacao ? 'Relatório VIP' : 'VIP BOLOES'; ?></title>
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        @media (max-width: 768px) {
            ...
            .tr-ganhador { background: rgba(16, 185, 129, 0.08) !important; border: 1px solid rgba(16, 185, 129, 0.3) !important; }
    ...
    <script>
        ...
        // --- FUNÇÃO LIMPAR TUDO ---
        function limparTudo() {
            if(confirm('Tem certeza? Isso apagará os resultados e a lista de jogos.')) {
                // Limpa campos de bola
                document.querySelectorAll('.input-bola').forEach(el => el.value = '');
                // Limpa textarea
                document.querySelector('textarea[name="lista"]').value = '';
                // Redireciona para limpar o POST (evita reenviar dados ao atualizar)
...
<div class="container">
    ...
    <div class="header-card">
        <h1><?php echo $modo_visualizacao ? '📊 RELATÓRIO VIP BOLOES 📊' : 'VIP BOLOES'; ?></h1>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_POST['lista'])) {
        ...
        if (!$modo_visualizacao) {
            ...
            file_put_contents("$pasta_recibos/$id_unico.json", json_encode($dados));
        ...
        foreach ($linhas as $linha) {
            ...
            echo "<tr $bg_tr>";
            echo "<td>
                    <div class='nome-player'>$nome_final</div>
