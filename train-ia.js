...
try {
    ...
    console.error("Erro ao ler FIREBASE_CREDENTIALS. Verifique o Secret no GitHub.");
    process.exit(1);
}
...
async function runTreinamento() {
    ...
    for (const loteria of loterias) {
        ...
        // Otimizado para 1000 para economizar cota do Firebase
    ...
    // --- CORREÇÃO DO PLACAR DE MARKETING ---
    console.log("🤖 Calculando Placar de Marketing...");
    const tresDiasAtras = new Date(); 
    tresDiasAtras.setDate(tresDiasAtras.getDate() - 3);

    const snapRecentes = await db.collection('resultados')
        .where('data', '>=', admin.firestore.Timestamp.fromDate(tresDiasAtras))
        .where('posicao', '==', 1)
        .get();

    let acertos = 0; 
    let loteriasHit = new Set();

    for (const loteria of loterias) {
        const docIA = await db.collection('analises_ia').doc(loteria).get();
        if(!docIA.exists) continue;
        const iaData = docIA.data();
        if(!iaData.geral) continue;
        
        const analise = iaData.geral;

        const top5 = tabelaDeGrupos.map(b => {
            let score = 25;
            if(analise.bichosAtrasados1Premio && analise.bichosAtrasados1Premio.find(x => x.grupo === b.grupo)) score += 30;
            if(analise.bichosFrequentes && analise.bichosFrequentes.find(x => x.grupo === b.grupo)) score += 20;
            if(analise.ciclos && analise.ciclos[b.grupo] && analise.ciclos[b.grupo].estourado) score += 20;
            return { grupo: b.grupo, score: score };
        }).sort((a,b) => b.score - a.score).slice(0, 5).map(b => b.grupo);

        snapRecentes.forEach(doc => {
            const res = doc.data();
            if (res.loteria === loteria && top5.includes(res.bichoGrupo)) { 
                acertos++; 
                loteriasHit.add(loteria.toUpperCase()); 
            }
        });
    }
