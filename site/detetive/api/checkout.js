...
export default async function handler(req, res) {
    // 1. Configuração de CORS (Permite que seu site converse com essa API)
    ...
    // Responde ao "sinal" do navegador
    ...
    try {
        ...
        // --- SUAS CREDENCIAIS YAMPI (CONFIGURADAS) ---
        const ALIAS = "store39";
        ...
        // 2. Monta o Payload para a Yampi
        const payload = {
            customer: {
                name: dados.nome,
                email: dados.email,
        ...
        // 4. Retorna a resposta para o HTML
        if (response.ok) {
            const retorno = { 
                sucesso: true, 
                metodo: dados.metodo,
                // Pega a URL de checkout caso precisemos redirecionar
                redirect_url: resYampi.data?.checkout_url || resYampi.data?.resource?.checkout_url 
            };
            
            if (dados.metodo === 'pix') {
                // Tenta achar o QR Code em locais diferentes da resposta
                const transaction = resYampi.data?.resource?.transactions?.[0] || resYampi.data?.transactions?.[0];
                retorno.qrcode_imagem = transaction?.pix_qrcode_url || null;
                retorno.qrcode_copicola = transaction?.pix_qrcode_text || null;
            }
